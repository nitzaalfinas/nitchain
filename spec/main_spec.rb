require File.expand_path '../spec_helper.rb', __FILE__

describe "NitChain application" do

    MINE = 0
    KEYSTORE_PATH    = "/Users/nitzaalfinas/DocB/app_group/blockchain/nitchain_user/data_rtest/user/keystore"
    MAIN_CONFIG_PATH = "/Users/nitzaalfinas/DocB/app_group/blockchain/nitchain_user/data_rtest/config"
    MAIN_POOL = 'http://localhost:4568'
    DATABASE_NAME = 'nitchain_test'

    # it 'should get /' do
    #     get '/'
    #
    #     puts last_response
    #
    #     expect(last_response).to be_ok
    # end

    # it 'should create a wallet' do
    #     get '/wallet/create'
    #
    #     puts last_response.body
    #
    #     expect(last_response).to be_ok
    # end

    # post /miner/pool/submit
    it 'membuat valid data dan diterima oleh miner pool' do
        MINE = 1
        json_payload = '{
                "data":{
                    "from":"Nx3476e5c2b698c4746bc184f96b47815baa352cc3",
                    "to":"Nx6a578ada9acd3eec09bd695e1b213c0671887c4e",
                    "amount":"120",
                    "time":1565822616
                },
                "hash":"5a8560f081098191ea5baa50d51e0ef7270fecb6",
                "enc":"\n",
                "sign":"\n",
                "pubkey":"-----BEGIN PUBLIC KEY-----\\nxwIDAQAB\n-----END PUBLIC KEY-----\n"
            }'

        post '/miner/pool/submit', json_payload, {format: 'json'}

        puts "xxxxxx"
        puts last_response.body

        #expect(last_response.body).should eq("5a8560f081098191ea5baa50d51e0ef7270fecb6")
    end

    # post /miner/pool/submit
    it 'tidak bisa masuk kedalam pool masih ada data yang sama dalam pool (from, to, amount)' do
        MINE = 1
        json_payload = '{
                "data":{
                    "from":"Nx3476e5c2b698c4746bc184f96b47815baa352cc3",
                    "to":"Nx6a578ada9acd3eec09bd695e1b213c0671887c4e",
                    "amount":"120",
                    "time":1565822616
                },
                "hash":"5a8560f081098191ea5baa50d51e0ef7270fecb6",
                "enc":"\n",
                "sign":"\n",
                "pubkey":"-----BEGIN PUBLIC KEY-----\\nxwIDAQAB\n-----END PUBLIC KEY-----\n"
            }'

        post '/miner/pool/submit', json_payload, {format: 'json'}

        expect(last_response.body).to eq("Transaction hash exist")
    end

    # # get /miner/mine/create_merkle
    # it "should create merkle tree" do
    #     # sebelum testing, harus dibuat dulu banyak data dalam database
    #     MINE = 1
    #
    #     get '/miner/mine'
    #
    #     puts last_response.body
    # end
end
